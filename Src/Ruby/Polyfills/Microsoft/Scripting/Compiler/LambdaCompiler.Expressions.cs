    ...
    partial class LambdaCompiler {
        ...
        private void EmitExpressionAsType(Expression node, Type type, CompilationFlags flags) {
            ...
            if (type == typeof(void)) {
                ...
                if (!TypeUtilsEx.AreEquivalent(node.Type, type)) {
                    EmitExpression(node);
                    Debug.Assert(TypeUtilsEx.AreReferenceAssignable(type, node.Type));
        ...
        internal static void ValidateLift(IList<ParameterExpression> variables, IList<Expression> arguments) {
            ...
            for (int i = 0, n = variables.Count; i < n; i++) {
                if (!TypeUtilsEx.AreReferenceAssignable(variables[i].Type, TypeUtilsEx.GetNonNullableType(arguments[i].Type))) {
            ...
            switch (nodeType) {
                ...
                case ExpressionType.GreaterThanOrEqual: {
                        ...
                        for (int i = 0, n = paramList.Length; i < n; i++) {
                            ...
                            if (TypeUtilsEx.IsNullableType(arg.Type)) {
                        ...
                        if (TypeUtilsEx.IsNullableType(resultType) && !TypeUtilsEx.AreEquivalent(resultType, mc.Type)) {
