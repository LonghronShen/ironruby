...
namespace System.Linq.Expressions.Compiler {
    ...
    internal partial class StackSpiller {
        ...
        internal static LambdaExpression AnalyzeLambda(LambdaExpression lambda) {
            return lambda.DynamicInvoke<LambdaExpression, LambdaExpression>("Accept", new object[] { new StackSpiller(Stack.Empty) });
        }
        ...
        internal Expression<T> Rewrite<T>(Expression<T> lambda) {
            ...
            if (body.Action != RewriteAction.None) {
                ...
                return (Expression<T>)Activator.CreateInstance(
                    typeof(Expression<T>), BindingFlags.NonPublic, null, new object[] {
                        newBody,
                        lambda.Name, 
                        lambda.TailCall, 
                        lambda.Parameters 
        ...
        private static void VerifyRewrite(Result result, Expression node) {
            ...
            Debug.Assert(
                TypeUtilsEx.AreReferenceAssignable(node.Type, result.Node.Type),
        ...
        private Result RewriteLogicalBinaryExpression(Expression expr, Stack stack) {
            ...
            if (action != RewriteAction.None) {
                ...
                expr = ExpressionsHelper.CreateBinaryExpression(
                    node.NodeType,
                    left.Node,
                    right.Node,
                    node.Type,
