...
namespace System.Linq.Expressions.Compiler {
    ...
    internal static class ILGen {
        ...
        internal static void EmitConvertToType(this ILGenerator il, Type typeFrom, Type typeTo, bool isChecked) {
            if (TypeUtilsEx.AreEquivalent(typeFrom, typeTo)) {
                return;
            }
            ...
            bool isTypeFromNullable = TypeUtilsEx.IsNullableType(typeFrom);
            bool isTypeToNullable = TypeUtilsEx.IsNullableType(typeTo);

            Type nnExprType = TypeUtilsEx.GetNonNullableType(typeFrom);
            Type nnType = TypeUtilsEx.GetNonNullableType(typeTo);
            ...
            if (typeFrom.IsInterface || // interface cast
               ...
               TypeUtilsEx.IsLegalExplicitVariantDelegateConversion(typeFrom, typeTo))
            {
            ...
            } else if (!(TypeUtilsEx.IsConvertible(typeFrom) && TypeUtilsEx.IsConvertible(typeTo)) // primitive runtime conversion
                       &&
        ...
        private static void EmitNumericConversion(this ILGenerator il, Type typeFrom, Type typeTo, bool isChecked) {
            bool isFromUnsigned = TypeUtilsEx.IsUnsigned(typeFrom);
            bool isFromFloatingPoint = TypeUtilsEx.IsFloatingPoint(typeFrom);
        ...
        private static void EmitNullableToNullableConversion(this ILGenerator il, Type typeFrom, Type typeTo, bool isChecked) {
            Debug.Assert(TypeUtilsEx.IsNullableType(typeFrom));
            Debug.Assert(TypeUtilsEx.IsNullableType(typeTo));
            ...
            Type nnTypeFrom = TypeUtilsEx.GetNonNullableType(typeFrom);
            Type nnTypeTo = TypeUtilsEx.GetNonNullableType(typeTo);
