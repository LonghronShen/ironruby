    ...
    partial class LambdaCompiler {
        ...
        private void EmitBinaryExpression(Expression expr, CompilationFlags flags) {
            ...
            if ((b.NodeType == ExpressionType.Equal || b.NodeType == ExpressionType.NotEqual) &&
                ...
                if (ConstantCheck.IsNull(b.Left) && !ConstantCheck.IsNull(b.Right) && TypeUtilsEx.IsNullableType(b.Right.Type)) {
                    EmitNullEquality(b.NodeType, b.Right, b.IsLiftedToNull);
                    return;
                }
                if (ConstantCheck.IsNull(b.Right) && !ConstantCheck.IsNull(b.Left) && TypeUtilsEx.IsNullableType(b.Left.Type)) {
        ...
        private void EmitNullEquality(ExpressionType op, Expression e, bool isLiftedToNull) {
            Debug.Assert(TypeUtilsEx.IsNullableType(e.Type));
        ...
        private void EmitBinaryOperator(ExpressionType op, Type leftType, Type rightType, Type resultType, bool liftedToNull) {
            bool leftIsNullable = TypeUtilsEx.IsNullableType(leftType);
            bool rightIsNullable = TypeUtilsEx.IsNullableType(rightType);
        ...
        private void EmitUnliftedBinaryOp(ExpressionType op, Type leftType, Type rightType) {
            Debug.Assert(!TypeUtilsEx.IsNullableType(leftType));
            Debug.Assert(!TypeUtilsEx.IsNullableType(rightType));
            ...
            switch (op) {
                ...
                case ExpressionType.AddChecked: 
                    if (TypeUtilsEx.IsFloatingPoint(leftType)) {
                        _ilg.Emit(OpCodes.Add);
                    } else if (TypeUtilsEx.IsUnsigned(leftType)) {
                ...
                case ExpressionType.Divide:
                    if (TypeUtilsEx.IsUnsigned(leftType)) {
        ...
        private void EmitLiftedBinaryOp(ExpressionType op, Type leftType, Type rightType, Type resultType, bool liftedToNull) {
            Debug.Assert(TypeUtilsEx.IsNullableType(leftType) || TypeUtilsEx.IsNullableType(rightType));
            switch (op) {
                case ExpressionType.And:
        ...
        private void EmitLiftedRelational(ExpressionType op, Type leftType, Type rightType, Type resultType, bool liftedToNull) {
            ...
            EmitBinaryOperator(
                op,
                TypeUtilsEx.GetNonNullableType(leftType),
                TypeUtilsEx.GetNonNullableType(rightType),
                TypeUtilsEx.GetNonNullableType(resultType),
                false
            );
        ...
        private void EmitLiftedBinaryArithmetic(ExpressionType op, Type leftType, Type rightType, Type resultType) {
            ...
            EmitBinaryOperator(op, TypeUtilsEx.GetNonNullableType(leftType), TypeUtilsEx.GetNonNullableType(rightType), TypeUtilsEx.GetNonNullableType(resultType), false);

            // construct result type
            ConstructorInfo ci = resultType.GetConstructor(new Type[] { TypeUtilsEx.GetNonNullableType(resultType) });
