...
namespace System.Linq.Expressions.Compiler {
    ...
    partial class LambdaCompiler {
        ...
        private void AddressOf(BinaryExpression node, Type type) {
            ...
            if (TypeUtilsEx.AreEquivalent(type, node.Type)) {
                EmitExpression(node.Left);
                EmitExpression(node.Right);
                Type rightType = node.Right.Type;
                if (TypeUtilsEx.IsNullableType(rightType)) {
                ...
                Type indexType = TypeUtilsEx.GetNonNullableType(rightType);
        ...
        private void AddressOf(IndexExpression node, Type type) {
            if (!TypeUtilsEx.AreEquivalent(type, node.Type) || node.Indexer != null) {
                EmitExpressionAddress(node, type);
                return;
            }
        ...
        private void AddressOf(UnaryExpression node, Type type) {
            ...
            Debug.Assert(type.IsValueType && !TypeUtilsEx.IsNullableType(type));
